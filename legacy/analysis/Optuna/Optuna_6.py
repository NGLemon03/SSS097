...
# CPCV 設定
CPCV_NUM_SPLITS = 10
CPCV_EMBARGO_DAYS = 30  # 自定義 embargo,單位：天
...
# 各部份分數權重（報酬 60%,其他 15%）
SCORE_WEIGHTS = dict(
    total_return=3.0,       # 高優先級: 完整回測報酬
    profit_factor=1.0,      # 高優先級: 盈虧因子
    wf_min_return=0.1,      # 降低次要指標權重
    sharpe_ratio=0.1,
    max_drawdown=0.1,
...
def _wf_min_return(strat: str, params: dict, data_source: str) -> float:
    """計算 walk_forward 期間最差報酬。"""
    try:
        df_price, df_factor = data_loader.load_data(TICKER, smaa_source=data_source)
        if df_price.empty:
            logger.warning(f"walk_forward 測試數據為空,策略: {strat}, 數據源: {data_source}, 參數: {params}")
            log_to_results("警告", f"walk_forward 測試數據為空,策略: {strat}, 數據源: {data_source}, 參數: {params}")
            return -np.inf
        ...
        for start, end in WF_PERIODS:
            ...
            if start_candidates.empty or end_candidates.empty:
                logger.warning(f"無效 walk_forward 期間: {start} → {end}, 數據範圍: {valid_dates[0]} 到 {valid_dates[-1]}")
                continue
            adjusted_start = start_candidates[0]
            adjusted_end = end_candidates[-1]
            if adjusted_start != pd.Timestamp(start) or adjusted_end != pd.Timestamp(end):
                logger.info(f"walk_forward 期間調整: {start} → {adjusted_start}, {end} → {adjusted_end}")
            valid_periods.append((adjusted_start, adjusted_end))

        if not valid_periods:
            logger.warning(f"無有效的 walk_forward 期間,策略: {strat}, 數據源: {data_source}, 參數: {params}")
            log_to_results("警告", f"無有效的 walk_forward 期間,策略: {strat}, 數據源: {data_source}, 參數: {params}")
            return -np.inf
        ...
        if not results:
            logger.warning(f"walk_forward 測試無結果,策略: {strat}, 數據源: {data_source}, 參數: {params}")
            log_to_results("警告", f"walk_forward 測試無結果,策略: {strat}, 數據源: {data_source}, 參數: {params}")
            return -np.inf
        ...
        for r in results:
            try:
                ...
                logger.warning(f"walk_forward 期間缺少 total_return,策略: {strat}, 數據源: {data_source}, 參數: {params}, 錯誤: {e}")
                log_to_results("警告", f"walk_forward 期間缺少 total_return,策略: {strat}, 數據源: {data_source}, 參數: {params}, 錯誤: {e}")
                continue
        return min(valid_returns) if valid_returns else -np.inf
    except Exception as e:
        logger.error(f"walk_forward 測試失敗,策略: {strat}, 數據源: {data_source}, 參數: {params}, 錯誤: {e}")
        log_to_results("錯誤", f"walk_forward 測試失敗,策略: {strat}, 數據源: {data_source}, 參數: {params}, 錯誤: {e}")
        return -np.inf
...
def objective(trial: optuna.Trial):
    ...
    if profit_factor < 2.5:
        logger.info(f"試驗被剔除: 盈虧因子 {profit_factor:.2f} < 2.5,策略: {strat}, 數據源: {data_source}")
        return -np.inf
    ...
    try:
        ...
        # 初始化 TimeSeriesSplit
        tscv = TimeSeriesSplit(n_splits=CPCV_NUM_SPLITS)
        folds = []
        ...
        for train_block, test_block in folds:
            ...
            if strategy_return <= bh_return or metrics.get('profit_factor', 1.0) < 2.0:
                fail_count += 1
                # 簡化 SRA
        ...
        # 動態門檻
        fail_ratio_threshold = 0.5 if total_ret > 10.0 else 0.4
        ...
        if avg_buy_dist > 20 or avg_sell_dist > 20:
            ...
            if fail_ratio > fail_ratio_threshold or pbo_score > pbo_threshold:
                penalty = min(0.25, fail_ratio + pbo_score / 2)  # 最多扣 25% 分數
                score *= (1 - penalty)
                ...
                penalty = min(0.3, max(fail_ratio, pbo_score / 2, oos_std / 10))
                score *= (1 - penalty)
            ...
            if len(trial_results) >= 15:
                ...
                if stab > 0.5:
                    ...
                    score *= (1 - min(penalty_mult, 0.15))  # 最多扣 15% 分數
            ...
            # 繪製 OOS 報酬分佈圖（每 100 次試驗）
            if trial.number % 100 == 0 and oos_returns:
                plt.figure(figsize=(10, 6))
                plt.hist(oos_returns, bins=20, edgecolor='black')
                plt.title(f'OOS 報酬分佈 - 試驗 {trial.number}')
                plt.xlabel('OOS Total Return')
                plt.ylabel('頻率')
                plt.savefig(cfg.RESULT_DIR / f'oos_distribution_trial_{trial.number}_{TIMESTAMP}.png')
                plt.close()
