...
def objective(trial: optuna.Trial):
    ...
    try:
        ...
        if trial_results:
            ...
            trial.study.set_user_attr("trial_results", trial_results)
            logger.info(f"已更新試驗結果，長度: {len(trial_results)}")
        ...
        log_data = {
            ...
            **params_flat,
            ...
            "raw_total_return": total_ret,  # 保留數值型
            "num_trades": n_trades,
        ...
        if not any(tuple(sorted(r.items()))[:len(params_key)] == params_key for r in trial_results):
            ...
            trial.study.set_user_attr("trial_results", trial_results)  # ★ 補回寫入
            logger.info(f"已追加備用邏輯試驗結果，長度: {len(trial_results)}")
        return score
...
if __name__ == "__main__":
    ...
    if args.data_source_mode == 'sequential':
        ...
        for data_source in DATA_SOURCES:
            ...
            safe_ds = sanitize(data_source)   # e.g. "Factor_TWII_2412_TW"
            optuna_sqlite = Path(cfg.RESULT_DIR) / f"optuna_{args.strategy}_{safe_ds}_{TIMESTAMP}.sqlite3"

            study = optuna.create_study(
                study_name=f"00631L_optuna_{args.strategy}_{data_source}",
                direction="maximize",
            ...
            merged_df = merge_correlations_simplified(df_results, correlation_dfs, args.strategy, TIMESTAMP)
            ...
            result_file = cfg.RESULT_DIR / f"optuna_best_params_{args.strategy}_{data_source.replace(' ', '_').replace('^', '')}_{TIMESTAMP}.json"
            pd.Series(results).to_json(result_file, indent=2)
            result_csv_file = cfg.RESULT_DIR / f"optuna_results_{args.strategy}_{data_source.replace(' ', '_').replace('^', '')}_{TIMESTAMP}.csv"
            ...
            event_csv_file = cfg.RESULT_DIR / f"optuna_events_{args.strategy}_{data_source.replace(' ', '_').replace('^', '')}_{TIMESTAMP}.csv"
        ...
        # random 或 fixed 模式
        optuna_sqlite = Path(cfg.RESULT_DIR) / f"optuna_{args.strategy}_{TIMESTAMP}.sqlite3"
