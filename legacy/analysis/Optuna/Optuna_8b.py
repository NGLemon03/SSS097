...
* 本腳本可直接放在 analysis/ 目錄後以 `python optuna_8.py` 執行。
...
* 命令列範例：
  python optuna_8.py --strategy RMA --n_trials 10000
  python optuna_8.py --strategy single --n_trials 10000
  python optuna_8.py --strategy dual --n_trials 10000
  python optuna_8.py --strategy ssma_turn --n_trials 10000
...
FIX01. 壓力測試基於完整回測的 Equity Curve，WF 測試維持獨立回測
依賴：
'''
import logging
...
# 初始化日誌設定
setup_logging()
logger = logging.getLogger("optuna_8")
...
def compute_period_return(equity: pd.Series, start, end) -> float:
...
def _backtest_once(strat: str, params: dict, trial_results: list, data_source: str, 
    ...
    try:
        ...
        if not bt or "metrics" not in bt:
            logger.error(f"回測結果無效，策略: {strat}, 數據源: {data_source}, bt: {bt}")
            return -np.inf, 0, 0.0, 0.0, 0.0, [], pd.Series()

        metrics = bt["metrics"]
        trades_df = bt.get("trades_df")
        equity_curve = bt.get("equity_curve")
        if trades_df is None or equity_curve.empty:
            logger.warning(f"回測未返回 trades_df 或 equity_curve，策略: {strat}, 數據源: {data_source}")
            return -np.inf, 0, 0.0, 0.0, 0.0, [], pd.Series()
...
def _wf_min_return(strat: str, params: dict, data_source: str) -> float:
    try:
        ...
        results = SSS.compute_backtest_for_periods(
            ...
            df_price=df_price,
            df_factor=df_factor,
            smaa_source=data_source  # 傳遞 smaa_source
        )
        valid_returns = []
...
def _stress_avg_return(strat: str, params: dict, data_source: str, valid_stress_periods: list, df_price: pd.DataFrame, equity_curve: pd.Series) -> float:
    if not valid_stress_periods:
        logger.warning(f"無有效的壓力測試期間,策略: {strat}, 數據源: {data_source}")
        return np.nan
    try:
        if df_price.empty or equity_curve.empty:
            logger.warning(f"壓力測試數據或 Equity Curve 為空,策略: {strat}, 數據源: {data_source}")
            return np.nan
        valid_dates = df_price.index.intersection(equity_curve.index)
        valid_returns = []
        for start, end in valid_stress_periods:
            total_return = compute_period_return(equity_curve, start, end)
            if np.isnan(total_return):
                logger.warning(f"壓力測試時段 {start} 至 {end} 無有效報酬")
                continue
            valid_returns.append(total_return)
            logger.info(f"壓力測試時段 {start} 至 {end}: 報酬={total_return:.2f}")
...
def objective(trial: optuna.Trial):
    ...
    if df_price.empty:
        ...
        return -np.inf
    df_factor.to_csv(f"v8_factor_data_{TIMESTAMP}.csv")
    valid_dates = df_price.index
    ...
    for start, end in STRESS_PERIODS:
        ...
        if start_candidates.empty or end_candidates.empty:
            ...
            continue
        adjusted_start = start_candidates[0]  # 使用暖機起始日
        adjusted_end = end_candidates[-1]
    ...
    total_ret, n_trades, sharpe_ratio, max_drawdown, profit_factor, trades, equity_curve = _backtest_once(
    ...
    if min_wf_ret == -np.inf:
        return -np.inf
    #FIX0002 : 平均壓力測試報酬_實際交易
    avg_stress_ret = _stress_avg_return(strat, params, data_source, valid_stress_periods, df_price, equity_curve)
    ...
    try:
        ...
        for train_block, test_block in folds:
            ...
            total_return = compute_period_return(equity_curve, test_start, test_end)
            if np.isnan(total_return):
                logger.warning(f"CPCV 時段 {test_start} 至 {test_end} 無有效報酬")
                continue
...
if __name__ == "__main__":
    ...
    optuna_sqlite = Path(cfg.RESULT_DIR) / f"optuna_8_{TIMESTAMP}.sqlite3"
    study = optuna.create_study(
        study_name=f"00631L_optuna_v8_{args.strategy}",
        direction="maximize",
    ...
    result_file = cfg.RESULT_DIR / f"optuna_8_best_params_{TICKER.replace('^','')}_{TIMESTAMP}.json"
    pd.Series(results).to_json(result_file, indent=2)
    logger.info(f"最佳參數已保存至 {result_file}")

    # 將 results_log 保存為 CSV
    df_results = pd.DataFrame(results_log)
    result_csv_file = cfg.RESULT_DIR / f"optuna_8_results_{TIMESTAMP}.csv"
    df_results.to_csv(result_csv_file, index=False, encoding='utf-8-sig', na_rep='0.0')
