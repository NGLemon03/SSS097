...
def compute_excess_return_in_stress_equity(equity: pd.Series, df_price: pd.DataFrame, stress_periods: list) -> float:
    ...
    return np.nanmean(excess) if excess and not all(np.isnan(excess)) else np.nan
...
def _backtest_once(strat: str, params: dict, trial_results: list, data_source: str, df_price: pd.DataFrame, df_factor: pd.DataFrame) -> Tuple[float, int, float, float, float, List, pd.Series, float]:
    ...
    try:
        ...
        trial_results.append({
            ...
            "stress_mdd": None,  # 稍後更新
            "excess_return_stress": None,  # 稍後更新
            **params_flat
        })
...
def objective(trial: optuna.Trial):
    ...
    params_flat = {f"param_{k}": v for k, v in params.items()}
    trial.study.set_user_attr("trial_results", trial_results)  # 確保寫回
    ...
    try:
        ...
        # 更新 top_trials（僅保留前 20 名）
        import heapq
        trial_entry = {
            "score": score,
            "equity_curve": equity_curve.to_json(),
            "trial_number": str(trial.number).zfill(5),
            "strategy": strat
        }
        if len(top_trials) < 20:
            heapq.heappush(top_trials, (score, trial_entry))
        else:
            heapq.heappushpop(top_trials, (score, trial_entry))
        ...
        mdd_s = 1 - minmax(abs(max_drawdown), 0, 0.3)
        stress_mdd = compute_stress_mdd_equity(equity_curve, valid_stress_periods) if 'equity_curve' in locals() else np.nan
        excess_return_stress = compute_excess_return_in_stress_equity(equity_curve, df_price, valid_stress_periods) if 'equity_curve' in locals() else np.nan
        stress_mdd_scaled = minmax(abs(stress_mdd), 0, 0.5) if not np.isnan(stress_mdd) else 0.0
        excess_return_scaled = minmax(excess_return_mean, -1, 1) if 'excess_return_mean' in locals() else 0.0
        pbo_score = pbo_score if 'pbo_score' in locals() else 0.0
        avg_p_value = 1.0
        avg_buy_dist = avg_buy_dist if 'avg_buy_dist' in locals() else 20.0
        avg_sell_dist = avg_sell_dist if 'avg_sell_dist' in locals() else 20.0
        robust_score = 0.4 * excess_return_scaled + 0.3 * (1 - stress_mdd_scaled) + 0.3 * (1 - pbo_score)
        score = (SCORE_WEIGHTS["total_return"] * tr_s + SCORE_WEIGHTS["profit_factor"] * pf_s +
                 SCORE_WEIGHTS["sharpe_ratio"] * sh_s + SCORE_WEIGHTS["max_drawdown"] * mdd_s +
                 SCORE_WEIGHTS["wf_min_return"] * wf_s + robust_score)
        trade_penalty = 0.05 * max(0, MIN_NUM_TRADES - n_trades)
        ...
        trial_details = (
            f"試驗 {str(trial.number).zfill(5)}, 策略: {strat}, 數據源: {data_source}, 分數: {score:.3f}, "
            f"參數: {params}, 總報酬={total_ret*100:.2f}%, 交易次數={n_trades}, 夏普比率={sharpe_ratio:.3f}, "
            f"最大回撤={max_drawdown:.3f}, 盈虧因子={profit_factor:.2f}, WF最差報酬={min_wf_ret:.2f}, "
            f"壓力平均報酬={avg_stress_ret:.3f}, 穩定性得分={stab:.2f}, Robust Score={robust_score:.3f}, "
            f"Excess Return in Stress={excess_return_stress:.3f}, Stress MDD={stress_mdd:.3f}, "
            f"PBO 分數={pbo_score:.2f}, SRA p-value={avg_p_value:.3f}, 買入距離低點={avg_buy_dist:.1f}天, "
            f"賣出距離高點={avg_sell_dist:.1f}天, 平均持倉天數={avg_hold_days:.1f}天"
        )
...
def plot_all_scatter(study, timestamp: str, data_source: str):
    ...
    if valid_stress_mdds:
        logger.info(f"Stress MDD 範圍: {min(valid_stress_mdds):.3f} ~ {max(valid_stress_mdds):.3f}, 型別: {type(valid_stress_mdds[0])}")
    if valid_excess_returns:
        logger.info(f"Excess Return Stress 範圍: {min(valid_excess_returns):.3f} ~ {max(valid_excess_returns):.3f}, 型別: {type(valid_excess_returns[0])}")
    safe_ds = sanitize(data_source)
    plot_files = [
...
def compute_equity_correlations_with_presets(
    ...
    """
    計算試驗前 top_n 名與 param_presets 的 equity curve 相關性。
    
    Args:
        trial_results: Optuna 試驗結果列表，包含 trial_number、score 等（不含 equity_curve）。
        param_presets: SSSv096.py 中的 param_presets 字典。
        top_n: 取分數前幾名試驗，預設 20。
        ticker: 股票代號，預設 00631L.TW。
        start_date: 數據起始日期。
        end_date: 數據結束日期。
        output_dir: 輸出目錄，儲存熱圖與 CSV。
    
    Returns:
        pd.DataFrame: 相關性矩陣，包含試驗與 param_presets 的 equity curve 相關性。
    """
    global top_trials
    TIMESTAMP = datetime.now().strftime('%Y%m%d_%H%M%S')
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # 1. 從 top_trials 獲取前 top_n 名試驗的 equity_curve
    sorted_trials = [entry for _, entry in top_trials[:top_n]]  # 僅取前 top_n 名
    logger.info(f"從 top_trials 篩選出 {len(sorted_trials)} 筆試驗進行相關性分析")

    # 2. 收集試驗的 equity curve
    df_list = []
    names = []
    for trial in sorted_trials:
        eq_json = trial.get('equity_curve')
    ...
    # 3. 生成 param_presets 的 equity curve
    ...
    # 4. 合併 equity curve 並計算相關性
    if not df_list:
        logger.error("無有效 equity curve，無法計算相關性")
        raise ValueError("無有效 equity curve")
    ...
    # 5. 儲存結果
    corr_file = output_dir / f"equity_corr_top_{top_n}_with_presets_{TIMESTAMP}.csv"
    ...
    plt.tight_layout()
    heatmap_file = output_dir / f"equity_corr_top_{top_n}_with_presets_{TIMESTAMP}.png"
...
if __name__ == "__main__":
    ...
    if args.data_source_mode == 'sequential':
        ...
        for data_source in DATA_SOURCES:
            ...
            safe_ds = sanitize(data_source)
            optuna_sqlite = cfg.RESULT_DIR / f"optuna_{args.strategy}_{safe_ds}_{TIMESTAMP}.sqlite3"
            ...
            # 生成散點圖
            plot_all_scatter(study, TIMESTAMP, data_source)
            ...
            result_file = cfg.RESULT_DIR / f"optuna_best_params_{args.strategy}_{safe_ds}_{TIMESTAMP}.json"
            result_file.parent.mkdir(parents=True, exist_ok=True)
            pd.Series(results).to_json(result_file, indent=2)
            result_csv_file = cfg.RESULT_DIR / f"optuna_results_{args.strategy}_{safe_ds}_{TIMESTAMP}.csv"
            ...
            event_csv_file = cfg.RESULT_DIR / f"optuna_events_{args.strategy}_{safe_ds}_{TIMESTAMP}.csv"
