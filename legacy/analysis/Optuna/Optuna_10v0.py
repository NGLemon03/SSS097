...
from scipy.stats import pearsonr  # 在檔案開頭 import 區塊新增
import seaborn as sns  # 在檔案開頭 import 區塊新增
version = 'optuna_10'
setup_logging()
...
def penalize_hold(avg_hold_days: float, target: float = 90, span: float = 30, max_penalty: float = 0.3) -> float:
    """
    計算平均持倉天數的懲罰值, 目標範圍 60 - 120 天.
    
    Args:
        avg_hold_days: 平均持倉天數.
        target: 目標持倉天數(中心值).
        span: 容忍範圍(±span).
        max_penalty: 最大懲罰比例(調高至 0.3 以嚴格限制).
    
    Returns:
        float: 懲罰值(0 到 max_penalty).
    """
...
def compute_correlations(results: list, param_keys: list, metric_keys: list) -> pd.DataFrame:
    """
    計算參數與指標之間的皮爾森相關係數.
...
def plot_correlation_heatmap(corr_df: pd.DataFrame, strategy_name: str, timestamp: str):
    """
    繪製相關係數熱圖.
...
def analyze_param_correlations(ticker: str, start_date: str, end_date: str, trial_results_all: list):
    """
    ...
    for strategy in ["single", "dual", "RMA", "ssma_turn"]:
        ...
        metric_keys = ["total_return", "sharpe_ratio", "max_drawdown", "profit_factor", "num_trades"]
...
def _backtest_once(strat: str, params: dict, trial_results: list, data_source: str, df_price: pd.DataFrame, df_factor: pd.DataFrame) -> Tuple[float, int, float, float, float, List, pd.Series, float]:
    ...
    try:
        if df_price.empty:
            ...
            return -np.inf, 0, 0.0, 0.0, 0.0, [], pd.Series(), 0.0  # 修正為 8 項
        ...
        avg_hold_days = _avg_holding_days(bt["trades"])
...
def objective(trial: optuna.Trial):
    ...
    # 選擇數據源
    data_source = trial.study.user_attrs.get("data_source", None)
    if not data_source:  # sequential 模式下由 study 傳入, random/fixed 模式下計算
    ...
    if len(trial_results) > 1000:
        trial_results = trial_results[-1000:]
    ...
    if trial.number == 0:
        ...
        out_csv = cfg.RESULT_DIR / f"optuna_v10_factor_data_{strat}_{safe_ds}_{TIMESTAMP}.csv"
    ...
    try:
        ...
        log_data = {
            ...
            **params_flat, # 加入攤平參數
            ...
            "avg_hold_days": f"{avg_hold_days:.1f}天"  # 新增
        }
        ...
        return score
def plot_all_scatter(study, timestamp: str):
    ...
    stress_mdds = [r.get("stress_mdd", 0.0) for r in results]
    excess_returns = [r.get("excess_return", 0.0) for r in results]
    ...
    plot_files = [
        f"mdd_vs_total_return_{timestamp}.png",
        f"sharpe_vs_total_return_{timestamp}.png",
        f"stress_mdd_vs_total_return_{timestamp}.png",
        f"excess_vs_total_return_{timestamp}.png"
    ]
    
    if len(valid_mdds) > 3:
        plt.figure(figsize=(10, 6))
        plt.scatter(valid_mdds, total_returns, alpha=0.4)
        plt.xlabel("MDD")
        plt.ylabel("Total Return")
        plt.title("Total Return vs MDD")
    ...
    if len(valid_sharpes) > 3:
        plt.figure(figsize=(10, 6))
        plt.scatter(valid_sharpes, total_returns, alpha=0.4)
        plt.xlabel("Sharpe Ratio")
        plt.ylabel("Total Return")
        plt.title("Total Return vs Sharpe Ratio")
    ...
    if len(valid_stress_mdds) > 3:
        plt.figure(figsize=(10, 6))
        plt.scatter(valid_stress_mdds, total_returns, alpha=0.4)
        plt.xlabel("Stress MDD")
        plt.ylabel("Total Return")
        plt.title("Total Return vs Stress MDD")
    ...
    if len(valid_excess_returns) > 3:
        plt.figure(figsize=(10, 6))
        plt.scatter(valid_excess_returns, total_returns, alpha=0.4)
        plt.xlabel("Excess Return in Stress")
        plt.ylabel("Total Return")
        plt.title("Total Return vs Excess Return in Stress")
...
if __name__ == "__main__":
    ...
    if args.data_source_mode == 'sequential':
        ...
        for data_source in DATA_SOURCES:
            ...
            optuna_sqlite = Path(cfg.RESULT_DIR) / f"optuna_v10_{args.strategy}_{data_source.replace(' ', '_').replace('^', '')}_{TIMESTAMP}.sqlite3"
            study = optuna.create_study(
                study_name=f"00631L_optuna_v10_{args.strategy}_{data_source}",
                direction="maximize",
                sampler=optuna.samplers.TPESampler(seed=42),
                pruner=optuna.pruners.MedianPruner(n_warmup_steps=20),
                storage=f"sqlite:///{optuna_sqlite}"
            )
            study.set_user_attr("data_source", data_source)  # 傳入固定數據源
            study.optimize(objective, n_trials=args.n_trials, n_jobs=1, show_progress_bar=True)
            plot_all_scatter(study, TIMESTAMP)
            # 處理試驗結果
            trial_results_all = study.user_attrs.get("trial_results", [])
            if len(trial_results_all) > 1000:
                trial_results_all = trial_results_all[-1000:]
            logger.info(f"開始分析參數與指標相關係數, 數據源: {data_source}")
            correlation_dfs = analyze_param_correlations(
                ticker=TICKER,
                start_date=cfg.START_DATE,
                end_date="2025-06-06",
                trial_results_all=trial_results_all
            )
            ...
            result_file = cfg.RESULT_DIR / f"optuna_v10_best_params_{args.strategy}_{data_source.replace(' ', '_').replace('^', '')}_{TIMESTAMP}.json"
            pd.Series(results).to_json(result_file, indent=2)
            result_csv_file = cfg.RESULT_DIR / f"optuna_v10_results_{args.strategy}_{data_source.replace(' ', '_').replace('^', '')}_{TIMESTAMP}.csv"
            ...
            event_csv_file = cfg.RESULT_DIR / f"optuna_v10_events_{args.strategy}_{data_source.replace(' ', '_').replace('^', '')}_{TIMESTAMP}.csv"
        ...
        # random 或 fixed 模式
        optuna_sqlite = Path(cfg.RESULT_DIR) / f"optuna_v10_{args.strategy}_{TIMESTAMP}.sqlite3"
        optuna_sqlite.parent.mkdir(parents=True, exist_ok=True)
        study = optuna.create_study(
            study_name=f"00631L_optuna_v10_{args.strategy}_{args.data_source_mode}",
            direction="maximize",
            sampler=optuna.samplers.TPESampler(seed=42),
            pruner=optuna.pruners.MedianPruner(n_warmup_steps=20),
            storage=f"sqlite:///{optuna_sqlite}"
        ...
        study.set_user_attr("trial_results", trial_results_all)
        # 合併相關係數至 results
        def merge_correlations_with_results(df_results, correlation_dfs, strategy, timestamp):
            """
            將相關係數合併至試驗結果 DataFrame。
            
            Args:
                df_results: 試驗結果 DataFrame (optuna_v10_results_{strategy}_{timestamp}.csv)
                correlation_dfs: 相關係數 DataFrame 字典，鍵為策略名稱
                strategy: 當前策略名稱
                timestamp: 時間戳記
            ...
            # 將相關係數轉為長格式
            corr_melted = corr_df.reset_index().melt(id_vars='index', var_name='metric', value_name='correlation')
            corr_melted['index'] = 'param_' + corr_melted['index']  # 統一參數名稱格式
            ...
            for param in corr_df.index:
                ...
                if param_col not in df_results.columns:
                    logger.warning(f"參數欄位 {param_col} 不存在於 results，跳過")
                    continue
                for metric in metric_mapping:
                    metric_col = metric_mapping[metric]
                    # 提取對應的相關係數
                    corr_value = corr_melted[
                        (corr_melted['index'] == param_col) & (corr_melted['metric'] == metric)
                    ]['correlation'].values
                    if len(corr_value) == 0:
                        logger.warning(f"參數 {param_col} 與指標 {metric} 無相關係數")
                        continue
                    # 新增相關係數欄位
                    corr_col_name = f'corr_{param}_{metric}'
                    merged_results[corr_col_name] = corr_value[0]
            
            # 儲存合併結果
            merged_file = cfg.RESULT_DIR / f"optuna_v10_results_with_corr_{strategy}_{timestamp}.csv"
            merged_results.to_csv(merged_file, index=False, encoding="utf-8-sig")
            logger.info(f"已將相關係數合併至 {merged_file}")
            log_to_results("相關係數合併", f"已將相關係數合併至 {merged_file}", merged_file=str(merged_file))
            
            return merged_results        

        # 執行合併
        df_results = pd.json_normalize(results_log, sep='_').loc[lambda d: d["Event Type"].str.contains("試驗結果")].sort_values("score", ascending=False)
        merged_df = merge_correlations_with_results(df_results, correlation_dfs, args.strategy, TIMESTAMP)
        ...
        result_file = cfg.RESULT_DIR / f"optuna_v10_best_params_{args.strategy}_{TIMESTAMP}.json"
        pd.Series(results).to_json(result_file, indent=2)
        result_csv_file = cfg.RESULT_DIR / f"optuna_v10_results_{args.strategy}_{TIMESTAMP}.csv"
        df_results = pd.json_normalize(results_log, sep='_').loc[lambda d: d["Event Type"].str.contains("試驗結果")].sort_values("score", ascending=False)
        df_results.to_csv(result_csv_file, index=False, encoding="utf-8-sig")
        logger.info(f"最佳參數已保存至 {result_file}")
        logger.info(f"試驗結果已保存至 {result_csv_file}")
        event_csv_file = cfg.RESULT_DIR / f"optuna_v10_events_{args.strategy}_{TIMESTAMP}.csv"
        df_events = pd.DataFrame(events_log)
        df_events.to_csv(event_csv_file, index=False, encoding='utf-8-sig', na_rep='0.0')
        logger.info(f"事件紀錄已保存至 {event_csv_file}")
