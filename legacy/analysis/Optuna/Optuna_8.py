...
def compute_period_return(df_price: pd.DataFrame, trades: list, start: str, end: str) -> tuple:
    ...
    try:
        ...
        period_trades = [
            t for t in trades
            if len(t) > 2 and pd.Timestamp(t[0]) >= start and pd.Timestamp(t[2]) <= end
        ]
        if not period_trades:
            logger.info(f"期間 {start} → {end} 無交易")
            return 0.0, 0
        returns = [t[1] for t in period_trades]  # t[1] 是每筆交易的報酬
    ...
    try:
        ...
        if strat == "ssma_turn":
            ...
            bt = SSS.backtest_unified(
                ...
                buy_dates=buys,
                sell_dates=sells,
                discount=COST_PER_SHARE / 100,  # 對齊 s1
    ...
    try:
        ...
        for i, r in enumerate(results):
            try:
                ...
                continue
        return 0.0 if not valid_returns else min(valid_returns)  # 對齊 s1
    except Exception as e:
...
def objective(trial: optuna.Trial):
    ...
    for start, end in STRESS_PERIODS:
        ...
        if start_candidates.empty or end_candidates.empty:
            ...
            continue
        adjusted_start = valid_dates[valid_dates >= pd.Timestamp(start)][0]
        adjusted_end = end_candidates[-1]
    ...
    try:
        ...
        if len(trial_results) >= 6:  # 跳過早期試驗
            ...
            trial_details = (
                ...
                f"WF最差報酬(分段)={min_wf_ret:.2f}, "
                f"壓力平均報酬(整段)={avg_stress_ret:.3f}, "
                f"穩定性得分={stab:.2f}, "
