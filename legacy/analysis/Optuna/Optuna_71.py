...
def _wf_min_return(strat: str, params: dict, data_source: str) -> float:
    try:
        ...
        for start, end in WF_PERIODS:
            ...
            logger.info(f"walk_forward 期間調整: {start} → {adjusted_start.strftime('%Y-%m-%d')}, {end} → {adjusted_end.strftime('%Y-%m-%d')}")
        ...
        for i, r in enumerate(results):
            try:
                ...
                if total_return == -np.inf:
                    logger.warning(f"walk_forward 時段 {valid_periods[i][0]} 至 {valid_periods[i][1]} 報酬無效")
                    continue
                logger.info(f"walk_forward 時段 {valid_periods[i][0]} 至 {valid_periods[i][1]} 報酬: {total_return:.2f}")
                valid_returns.append(total_return)
            except (KeyError, TypeError) as e:
                logger.warning(f"walk_forward 期間缺少 total_return,策略: {strat}, 數據源: {data_source}, 時段: {valid_periods[i]}, 錯誤: {e}")
                log_to_results("警告", f"walk_forward 期間缺少 total_return,策略: {strat}, 數據源: {data_source}, 時段: {valid_periods[i]}, 錯誤: {e}")
                continue
        if not valid_returns:
            logger.error(f"walk_forward 無有效報酬,策略: {strat}, 數據源: {data_source}, 參數: {params}")
            return -np.inf
        logger.info(f"walk_forward 報酬列表: {valid_returns}")
