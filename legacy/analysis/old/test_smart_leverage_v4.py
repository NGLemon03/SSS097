...
def clean_data(df):
    ...
    # 3. 強制轉數值 (修復 TypeError: str / str 的關鍵!)
    if 'close' in df.columns:
        # errors='coerce' 會把無法轉成數字的變成 NaN (例如表頭或怪字元)
        df['close'] = pd.to_numeric(df['close'], errors='coerce')
    
    # 4. 移除無效資料 (日期無效 或 收盤價無效)
...
def main():
    ...
    if df_target.empty:
        ...
        return
    print(f"   👉 {TARGET_ETF} 有效: {df_target.index[0].date()} ~ {df_target.index[-1].date()}")
    ...
    if df_safe.empty:
        print(f"❌ 無法載入 {SAFE_ETF} (資料為空或格式錯誤)")
        return
    print(f"   👉 {SAFE_ETF}    有效: {df_safe.index[0].date()} ~ {df_safe.index[-1].date()}")
