...
logger = logging.getLogger("OSv2")  # 使用專屬 logger
# 動態生成走查區間
...
for source in smaa_sources:
    ...
    logger.info(f"數據源 {source} 的 df_price 長度: {len(df_price)}, 日期範圍: {df_price.index.min()} 到 {df_price.index.max()}")
    if not df_factor.empty:
        logger.info(f"因子數據長度: {len(df_factor)}, 欄位: {df_factor.columns}")
        logger.info("因子數據統計:")
        logger.info(df_factor.describe())
        logger.info("因子數據頭部:")
        logger.info(df_factor.head())
    ...
    with st.spinner("正在執行回測..."):
        ...
        if equity_curves.empty:
            ...
            for start, end in [(p['test'] if isinstance(p, dict) else p) for p in periods]:
                ...
                logger.info(f"計算走查區間 {adjusted_start} 至 {adjusted_end} 的報酬率")
