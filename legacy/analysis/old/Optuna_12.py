...
version = 'optuna_12'
setup_logging()
logger = logging.getLogger('optuna_12')
# 設定 matplotlib 字體
...
pct_threshold_self = 10 # pick_topN_by_diversity 的歐式距離門檻
...
def _backtest_once(strat: str, params: dict, trial_results: list, data_source: str, df_price: pd.DataFrame, df_factor: pd.DataFrame, trial: optuna.Trial = None) -> Tuple[float, int, float, float, float, List, pd.Series, float]:
    try:
        ...
        if not bt or "metrics" not in bt or any(pd.isna(bt["metrics"].get(k)) for k in ["total_return", "num_trades"]):
...
def validate_stress_periods(df_price: pd.DataFrame, stress_periods: List[Tuple[str, str]], warmup_days: int) -> List[Tuple[pd.Timestamp, pd.Timestamp]]:
    """
    驗證壓力測試期間，確保在數據範圍內且足夠長。
    ...
    Returns:
        List[Tuple[pd.Timestamp, pd.Timestamp]]: 有效壓力測試期間。
    """
    valid_dates = df_price.index
    valid_periods = []
    for start, end in stress_periods:
...
def append_trial_result(trial_results: List[Dict], trial: optuna.Trial, params: Dict, score: float,
    ...
    """
    追加試驗結果至 trial_results 並更新 study.user_attrs，同時將 equity_curve_json 儲存至獨立 JSON 檔案。
    """
    params_flat = {f"param_{k}": v for k, v in params.items()}
    record = {
    ...
    # 將 equity_curve_json 儲存至獨立 JSON 檔案
    ...
    with lock:
        ...
        equity_data[record["trial_number"]] = equity_curve.to_json()
        with open(equity_json_file, 'w', encoding='utf-8') as f:
            json.dump(equity_data, f, indent=2)
        logger.debug(f"已將 trial {record['trial_number']} 的 equity_curve 儲存至 {equity_json_file}")
...
def objective(trial: optuna.Trial):
    ...
    if df_price.empty:
        ...
        log_trial_details("錯誤", f"價格數據為空, 策略: {strat}", 
    ...
    valid_stress_periods = validate_stress_periods(df_price, STRESS_PERIODS, warmup_days)
    if not valid_stress_periods:
        logger.error(f"無有效壓力測試時段, 策略: {strat}")
        log_trial_details("錯誤", f"無有效壓力測試時段, 策略: {strat}", 
                         trial, params, -np.inf, pd.Series(), strat, data_source, {})
        append_trial_result(trial_results, trial, params, -np.inf, pd.Series(), strat, data_source)
        return -np.inf
    ...
    # 進一步放寬篩選條件 - 讓更多試驗通過
    MIN_NUM_TRADES = 2  # 從 5 降到 2
    if total_ret == -np.inf or not (MIN_NUM_TRADES <= n_trades <= MAX_NUM_TRADES) or total_ret <= 0.2 or max_drawdown < -0.8 or profit_factor < 0.1:
    ...
    try:
        ...
        log_trial_details("試驗被剔除", f"CPCV/OOS 計算失敗: {e}, 策略: {strat}",
                         trial, params, -np.inf, equity_curve, strat, data_source, {
                             "total_return": total_ret,
                             "num_trades": n_trades,
                             "sharpe_ratio": sharpe_ratio,
...
def compute_equity_correlations_with_presets(
    ...
    sns.heatmap(
        corr_matrix,
        annot=False,
        cmap="RdYlGn",  # 更改為高反差多色色盤
        vmin=vmin,
        vmax=vmax,
        center=0,
        cbar_kws={'ticks': np.linspace(vmin, vmax, 11)}  # 自適應刻度，11 個刻度點
    )
