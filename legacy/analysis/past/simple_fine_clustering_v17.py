...
OUTPUT_DIR = 'results/simple_fine_clustering'
...
def simple_fine_clustering(df, strategy, datasource):
    """簡化細粒度分群：專門針對小群組需求"""
    print(f"\n=== {strategy} - {datasource} 簡化細粒度分群 ===")
...
def analyze_clustering_quality(clustering_result, df, strategy, datasource):
    """分析分群品質"""
...
def create_visualizations(df, clustering_result, quality_analysis, strategy, datasource):
    """創建可視化"""
    ...
    if len(available_cols) >= 2:
        ...
        ax3.set_title(f'最終分群PCA散點圖\n(顯示前20個群組)')
        ax3.legend()
        ax3.grid(True, alpha=0.3)
    
    # 4. 最終推薦
    ax4 = axes[1, 1]
    ax4.text(0.1, 0.9, f'策略: {strategy}', fontsize=12, transform=ax4.transAxes)
    ax4.text(0.1, 0.8, f'方法: {method}', fontsize=12, transform=ax4.transAxes)
    ax4.text(0.1, 0.7, f'總群組數: {len(final_sizes)}', fontsize=12, transform=ax4.transAxes)
    ax4.text(0.1, 0.6, f'平均群組大小: {quality_analysis["mean_size"]:.1f}', fontsize=12, transform=ax4.transAxes)
    ax4.text(0.1, 0.5, f'最大群組大小: {quality_analysis["max_size"]}', fontsize=12, transform=ax4.transAxes)
    ax4.text(0.1, 0.4, f'小群組比例: {quality_analysis["small_ratio"]:.3f}', fontsize=12, transform=ax4.transAxes)
    ax4.text(0.1, 0.3, f'顯著特徵數: {quality_analysis["n_significant_features"]}', fontsize=12, transform=ax4.transAxes)
    
    # 推薦信息
    if quality_analysis['small_ratio'] >= 0.7:
        ax4.text(0.1, 0.2, '✅ 小群組比例良好', fontsize=12, transform=ax4.transAxes, color='green')
    else:
        ax4.text(0.1, 0.2, '⚠️ 小群組比例偏低', fontsize=12, transform=ax4.transAxes, color='orange')
    ...
    plt.tight_layout()
    plt.savefig(f'{OUTPUT_DIR}/{strategy}_{datasource}_simple_fine_clustering.png', 
...
def main():
    ...
    for (strategy, datasource), df in data_dict.items():
        ...
        # 簡化細粒度分群
        clustering_result = simple_fine_clustering(df, strategy, datasource)
        
        # 分析分群品質
        quality_analysis = analyze_clustering_quality(clustering_result, df, strategy, datasource)
        
        # 創建可視化
        create_visualizations(df, clustering_result, quality_analysis, strategy, datasource)
        
        # 保存結果
        result_df = df.copy()
        result_df['simple_fine_cluster'] = clustering_result['final_labels']
        
        output_file = f'{OUTPUT_DIR}/{strategy}_{datasource}_simple_fine_clustered.csv'
        result_df.to_csv(output_file, index=False)
        
        all_recommendations[(strategy, datasource)] = {
            'method': clustering_result['method'],
            'total_clusters': len(clustering_result['final_sizes']),
